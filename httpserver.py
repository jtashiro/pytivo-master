...
class TivoHTTPHandler(BaseHTTPServer.BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if 'multipart/form-data' in ctype:
            # multipart/form-data handling would require additional parsing
            # For now, fall back to simple query string parsing
            length = int(self.headers.get('content-length', 0))
            qs = self.rfile.read(length)
            query = parse_qs(qs.decode('utf-8'), keep_blank_values=1)
        else:
            length = int(self.headers.get('content-length', 0))
            qs = self.rfile.read(length)
            query = parse_qs(qs.decode('utf-8'), keep_blank_values=1)
    ...
    def send_fixed(self, page, mime, code=200, refresh=''):
        ...
        if squeeze:
            out = StringIO()
            with gzip.GzipFile(mode='wb', fileobj=out) as gz:
                gz.write(page)
            page = out.getvalue()
            out.close()
