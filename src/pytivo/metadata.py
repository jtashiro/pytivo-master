...
from . import config
from .plugins.video import transcode
from . import turing
...
def from_moov(full_path):
    ...
    try:
        if isinstance(full_path, bytes):
            full_path_str = full_path.decode('utf-8')
        else:
            full_path_str = full_path
        mp4meta = mutagen.File(full_path_str)
        assert(mp4meta)
    except:
        mp4_cache[full_path] = {}
        return {}
    ...
    isTVShow = False
    if 'stik' in mp4meta:
        isTVShow = (mp4meta['stik'] == mutagen_mp4.MediaKind.TV_SHOW)
    else:
...
def from_dvrms(full_path):
    ...
    try:
        if isinstance(full_path, bytes):
            ...
            full_path_str = full_path
        rawmeta = mutagen.File(full_path_str)
        assert(rawmeta)
    except:
...
def from_eyetv(full_path):
    ...
    metadata = {}
    if isinstance(full_path, bytes):
        path = os.path.dirname(full_path.decode('utf-8'))
    else:
...
def basic(full_path, mtime=None):
    ...
    if not mtime:
        if isinstance(full_path, bytes):
            mtime = os.path.getmtime(full_path.decode('utf-8'))
        else:
...
def _parse_nfo(nfo_path, nfo_data=None):
    ...
    if nfo_data is None:
        with open(nfo_path, 'r') as f:
            nfo_data = [line.strip() for line in f]
    xmldoc = None
    try:
