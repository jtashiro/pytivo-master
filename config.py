...
def get_bin(fname):
    ...
    # Check if a specific path is configured
    config_path_bad = False
    if config.has_option('Server', fname):
        ...
        if os.path.exists(fpath) and os.path.isfile(fpath):
            ...
            config_path_bad = True
            configured_path = fpath
    ...
    for path in ([os.path.join(os.path.dirname(__file__), 'bin')] +
        ...
        if os.path.exists(fpath) and os.path.isfile(fpath):
            bin_paths[fname] = fpath
            if config_path_bad:
                logger.info('Configured %s path not found: %s, using: %s' % (fname, configured_path, fpath))
            return fpath

    # Not found anywhere
    if config_path_bad:
        logger.error('Bad %s path: %s (not found in PATH either)' % (fname, configured_path))
    else:
