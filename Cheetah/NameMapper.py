...
ClassType = type
TypeType = type
# In Python 3, there's no InstanceType - everything is a type
# We'll use object as a placeholder since the check is for exclusion
InstanceType = type
...
def hasKey(obj, key):
    ...
    # Python 3: Use 'in' operator for dict-like objects (no has_key method)
    try:
        if key in obj:
            return True
...
def valueForKey(obj, key):
    # Python 3: Try dict-style access first
    try:
        return obj[key]
